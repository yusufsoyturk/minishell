...
/*   Updated: 2025/07/12 17:02:12 by ktoraman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	heredoc_child(t_free *free_data, int pfd[2], 
									t_command *cmd, t_command *free_cmd)
{
	char	*raw;
	char	*expanded;
	t_redir	*redir;
	...
	while (1)
	{
		...
		expanded = heredoc_expand(redir, raw, free_data->env_list, free_data->mini);
		free(raw);
		heredoc_write_line(expanded, pfd[1]);
	}
	free_max(free_data->mini, free_data->env_list, free_cmd);
	free(free_data);
	close(pfd[1]);
	exit(0);
}
